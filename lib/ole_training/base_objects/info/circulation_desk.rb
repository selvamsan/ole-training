...
class CirculationDesk < InfoObject
  ...
  def initialize(desk_code='')
    raise OleTraining::Error,"Config file is empty: config/institutional/circulation.yml" if File.zero?('config/institutional/circulation.yml')
    src_ary     = YAML.load_file('config/institutional/circulation.yml')
    info_hsh    = desk_code.empty? ?
        src_ary.sample :
        src_ary.find {|desk| desk[:code] == desk_code}
    raise OleTraining::Error,"Circulation desk not found.#{" (Given #{desk_code})" unless desk_code.empty?}" if info_hsh.nil?
    @code       = info_hsh[:code]
    @name       = info_hsh[:name]
