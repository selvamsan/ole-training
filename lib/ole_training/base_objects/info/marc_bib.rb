...
class MarcBib < InfoObject
  ...
  #
  def to_file(opts={})
    raise OleTraining::Error,"MARC::Record not found." unless @record.is_a?(MARC::Record)
    # TODO Create :format option to enable Marc-XML support.
    defaults = {      
      :filename         => "#{OleTraining.timestamp}-#{@title}.mrc",
    ...
    if File.exists?("data/uploads/mrc/#{@filename}") && !opts[:force?]
      raise OleTraining::Error,"MARC file already exists, use the :force? => true option to overwrite.\n(Given: #{filename})."
    end
